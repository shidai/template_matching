...
int get_toa (double s[1024], double p[1024], double *phasex, double *errphasex)
{
    int nphase=1024;
	...
	double amp_s[NP],amp_p[NP];  // elements for calculating A7
	double phi_s[NP],phi_p[NP];

	preA7(&k, amp_s, amp_p, phi_s, phi_p, s, p, nphase);
	...
	if (d>=nphase/2)
	{
		...
		while (A7(up_phase, amp_s, amp_p, phi_s, phi_p, k)*A7(low_phase, amp_s, amp_p, phi_s, phi_p, k)>0.0)
		{
		    up_phase+=step;
		    low_phase-=step;
		}
    ...
    double phase,b;
    phase=zbrent(A7, low_phase, up_phase, 1.0e-16, amp_s, amp_p, phi_s, phi_p, k);
    //phase=zbrent(A7, -1.0, 1.0, 1.0e-16);
    //phase=zbrent(A7, -0.005, 0.005, 1.0e-16);
    b=A9(phase, amp_s, amp_p, phi_s, phi_p, k);
    //a=A4(b);
	...
	error(phase,b,&errphase,&errb, amp_s, amp_p, phi_s, phi_p, k);
